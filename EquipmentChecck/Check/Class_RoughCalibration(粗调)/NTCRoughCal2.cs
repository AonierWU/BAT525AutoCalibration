...
namespace TestSystem_Pack
{
    ...
    {
        public bool NTCRoughCal2(string Box, double NtcAcc, string CurAccResRegulation, string CheckType, string DeviceType, bool ReadEEPROM, bool RoughCheck, out string Point, out string strADCData1, out string strADCData2, out string strADCData3)
        {

            int[] CaliValue = { 2000, 5000, 10000, 15000, 20000 };//静态电流20000nA校正点
            ...
            for (int j = 0; j < 5; j++)
            {
                Vol_ADC_Add_L[j] = 0x6E + j * 2;// addr

                Vol_ADC_Add_L1[j] = 0xC2 + j * 4;// addr
                Vol_ADC_Add_L2[j] = 0xC2 + j * 4 + 2;// addr
            ...
            try
            {
                if (ReadEEPROM)
                {
                    if (!StopStatus)
                    {
                        if (!com.MixNTCCalibration())
                            return false;
                        if (!com.MixNTCRange(0x01))
                            return false;
                        strADCData1 = "NTC20KADC值1;";
                        strADCData2 = "NTC20KADC值2;";
                        strADCData3 = "NTC20KADC值3;";
                ...
                if (RoughCheck)
                {
                    if (!StopStatus)
                    {
                        ...
                        if (!com.MixNTCCalibration())
                            return false;
                        if (!com.IDRResCal_RY(10000, true))
                            return false;
                        if (!com.MixNTCRange(NtcRange))//00--2000R，01--20K，02--200K, 03--3M
                            return false;
                        if (!com.MixReadNTCValue(0x00, NtcRange, out NtcCaliTesterValue))
                            return false;
                        Err1 = NtcCaliTesterValue - 10000;
                        if (Math.Abs(Err1) < 1000)
                            Result = "√";
                        else
                            Result = "×";
                        UpdateUidelegate(DeviceType, Box, "NTC2粗调点检", "10000", "", NtcCaliTesterValue.ToString("f2"), Err1.ToString(), "", "1000", Result);
                        if (Result == "×")
                        {
                            Point = "误差比对超出1000Ω";
                            return false;
