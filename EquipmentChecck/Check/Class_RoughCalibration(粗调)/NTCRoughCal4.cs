...
namespace TestSystem_Pack
{
    ...
    {
        public bool NTCRoughCal4(string Box, double NtcAcc, string CurAccResRegulation, string CheckType, string DeviceType, bool ReadEEPROM, bool RoughCheck, out string Point, out string strADCData1, out string strADCData2, out string strADCData3)
        {
           ...
           // double Err2 = 0;
            ...
            try
            {
                if (ReadEEPROM)
                {
                    if (!StopStatus)
                    {
                        if (!com.MixNTCCalibration())
                            return false;
                        if (!com.MixNTCRange(0x03))
                            return false;
                        strADCData1 = "NTC3000KADC值1;";
                        strADCData2 = "NTC3000KADC值2;";
                        strADCData3 = "NTC3000KADC值3;";
                ...
                if (RoughCheck)
                {
                    if (!StopStatus)
                    {
                        ...
                        if (!com.MixNTCCalibration())
                            return false;
                        if (!com.IDRResCal_RY(1000000, true))
                            return false;
                        if (!com.MixNTCRange(NtcRange))//00--2000R，01--20K，02--200K, 03--3M
                            return false;
                        if (!com.MixReadNTCValue(0x00, NtcRange, out NtcCaliTesterValue))
                            return false;
                        Err1 = NtcCaliTesterValue - 1000000;
                        if (Math.Abs(Err1) < 100000)
                            Result = "√";
                        else
                            Result = "×";
                        UpdateUidelegate(DeviceType, Box, "NTC2粗调点检", "1000000", "", NtcCaliTesterValue.ToString("f2"), Err1.ToString(), "", "100000", Result);
                        if (Result == "×")
                        {
                            Point = "误差比对超出100000Ω";
                            return false;
            ...
            finally
            {
                ...
                {
                    com.MixNTCEnable();
                    com.IDRResCal_RY(1000000, false);
                }
            }
        }
    }
