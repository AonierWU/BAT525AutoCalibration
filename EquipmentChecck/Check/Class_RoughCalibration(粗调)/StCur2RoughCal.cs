...
namespace TestSystem_Pack
{
    ...
    {
        public bool StCur2RoughCal(string Box, double CurAcc, string CurAccResRegulation,string CheckType, string DeviceType, bool ReadEEPROM, bool RoughCheck, out string Point, out string strADCData)
        {
                ...
                if (ReadEEPROM)
                {
                    if (!StopStatus)
                    {
                        ...
                        for (int s = 0; s < CaliValue.Length; s++)
                        {
                            ...
                            if (!com.MixStCurRange(0x01, CurRange))
                                return false;
                            ...
                            UpdateUidelegate(DeviceType, Box, "读2000uA静态电流EEPROM", CaliValue[s].ToString(), "", "", "", "", "", Result);
                            //if (Result == "×")
                            //    return false;
                        }
                    }
                ...
                if(RoughCheck)
                {
                    if (!StopStatus)
                    {
                        ...
                        if (!com.StCurCal_RY("2000uA", true))
                            return false;
                        if (!com.MixStCurRange(0x01, 0x02))//StCurRange（R,range）静态的电阻R=01 50R ，02 500R ，03 5000R ，04 50K,range=0-3
                            return false;
                        if (!com.MixSetCellVoltValue((2000)))//设定电压
                            return false;
                        if (!com.MixReadStCurValue(0x00, 0x02, out CellCurCaliTesterValue))
                            return false;
                        Err1 = CellCurCaliTesterValue - 1000;
                        if (CellCurCaliTesterValue > 600)//只要读出来的值大于设定值的0.6即可
                            Result = "√";
                        else
                            Result = "×";
                        UpdateUidelegate(DeviceType, Box, "静态电流2000uA粗调点检", "1000", CellCurCaliDMMValue.ToString("f2"), CellCurCaliTesterValue.ToString("f2"), Err1.ToString("f2"), "", "200", Result);
                        if (Result == "×")
                        {
                            Point = "误差比对超出200uA";
                            return false;
