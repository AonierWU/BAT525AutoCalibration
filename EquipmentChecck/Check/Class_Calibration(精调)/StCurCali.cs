...
namespace TestSystem_Pack
{
    ...
    {
        public bool StCurCali(string Box, double CurCaliAcc, double CurCheckAcc, string CurAccResRegulation, string CheckType, string DeviceType, bool CaliType, out string Point)
        {
            ...
            //List<byte> CaliDH2Value = new List<byte> { };
            byte CurRange;
            Point = "未设初始值";
            ...
            //double CellVoltValue = 0.0f;
            //double Judge = 0.5;
            //int R = 2500;
            switch (DeviceType)
            {
                case "BAT525G":
                    int[] s = new int[] { 0, 3, 6, 10, 10, 20, 40, 60, 60, 80, 100, 120, 120, 150, 180, 200 };
                    CaliValue.AddRange(s);

                    double[] v1 = new double[] { 0, 7.5, 15, 25, 25, 50, 100, 150, 150, 200, 250, 300, 300, 375, 450, 500 };
                    ...
                    break;
                case "BAT525H":
                    int[] h = new int[] { 0, 3, 6, 10, 10, 20, 40, 60, 60, 80, 100, 120, 120, 150, 180, 200 };
                    CaliValue.AddRange(h);

                    double[] v2 = new double[] { 0, 7.5, 15, 25, 25, 50, 100, 150, 150, 200, 250, 300, 300, 375, 450, 500 };
            ...
            try
            {
                ...
                if (!StopStatus)
                {
                    ...
                    for (int s = 0; s < CaliValue.Count; s++)
                    {
                        ...
                        UpdateUidelegate(DeviceType, Box, "STC_200uA点检", CaliValue[s].ToString(), "", CellCurCaliTesterValue1.ToString(), Err1.ToString("f2"), "", CurCheckAcc.ToString(), Result);
                        if (s == CaliValue.Count - 1)
                        {
