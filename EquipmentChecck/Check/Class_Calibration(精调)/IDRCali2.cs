...
namespace TestSystem_Pack
{
    ...
    {
        public bool IDRCali2(string Box, double NtcCaliAcc, double NtcCheckAcc, string CurAccResRegulation, string CheckType, string DeviceType, bool CaliType, out string Point)
        {
            ...
            try
            {
                ...
                if (!StopStatus)
                {
                    ...
                    for (int s = 0; s < CaliValue.Count; s++)
                    {
                        ...
                        if (!com.IDRResCal_RY(CaliValue[s], true))
                            return false;
                        if (!com.MixIDRRange(0x01))//00--2000R，01--20K，02--200K, 03--3M
                            return false;
                        System.Threading.Thread.Sleep(100);
                        for (int i = 0; i < 5; i++)
                        {
                        ...
                        UpdateUidelegate(DeviceType, Box, "IDR电阻_10K点检", CaliValue[s].ToString(), "", NTCCheckTesterValue.ToString(), Err1.ToString("f2"), "", dblResAcc.ToString(), Result);
                        if (!com.IDRResCal_RY(CaliValue[s], false))
                            return false;
                        if (s == CaliValue.Count - 1)
                        {
