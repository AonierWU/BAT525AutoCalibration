...
namespace TestSystem_Pack
{
    ...
    {
        public bool NTCCali2(string Box, double NtcCaliAcc, double NtcCheckAcc, string CurAccResRegulation, string CheckType, string DeviceType, bool CaliType, out string Point)
        {
            ...
            {
                case "BAT525G":
                    int[] s = new int[] { 2000, 5000, 10000, 15000, 20000 };
                    CaliValue.AddRange(s);

                    refADCDH = 234;
                    Vol_ADC_Add_L = new int[5];//存放DAC设置值的低地址
                    Vol_ADC_Add_L1 = new int[5];//存放ADC读取值的低地址
                    Vol_ADC_Add_L2 = new int[5];
                    for (int j = 0; j < 5; j++)
                    {
                    ...
                    break;
                case "BAT525H":
                    int[] h = new int[] { 2000, 5000, 10000, 15000, 20000 };
                    ...
                    break;
                case "BAT525C":
                    int[] c = new int[] { 1000, 2000, 4000, 6000, 8000, 10000 };
                    CaliValue.AddRange(c);
  
                    Vol_ADC_Add_L = new int[6];//存放DAC设置值的低地址
                    for (int j = 0; j < 6; j++)
                    {
                        Vol_ADC_Add_L[j] = 0x6D + j * 3;// addr

                    }
                    break;
                case "BAT525D":
                    int[] d = new int[] { 1000, 2000, 4000, 6000, 8000, 10000 };
            ...
            try
            {
                ...
                if (!StopStatus)
                {
                    ...
                    for (int s = 0; s < CaliValue.Count; s++)
                    {
                        NtcRange = 0x01;
                        //if (DeviceType == "BAT525C" || DeviceType == "BAT525D")
                        //    NtcAcc = CaliValue[s] * 0.001 + FSValue;
                        //else
