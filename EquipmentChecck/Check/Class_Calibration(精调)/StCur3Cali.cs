...
namespace TestSystem_Pack
{
    ...
    {
        public bool StCur3Cali(string Box, double CurCaliAcc, double CurCheckAcc, string CurAccResRegulation, string CheckType, string DeviceType, bool CaliType, out string Point)
        {
            ...
            try
            {
                ...
                if (!StopStatus)
                {
                    ...
                    for (int s = 0; s < CaliValue.Count; s++)
                    {
                        ...
                        if (!com.MixStCurRange(0x04, CurRange))//StCurRange（R,range）静态的电阻R=01 50R ，02 500R ，03 5000R ，04 50K,range=0-3
                            return false;
                        ...
                        if (!com.MixReadStCur_ADCValue(CurRange, out VoltDL, out VoltDH))
                            break;
                        if (Math.Abs((VoltDH2 * 256 + VoltDL2) - (VoltDH * 256 + VoltDL)) <= 800)
                        {
                ...
                if (!StopStatus)
                {
                    ...
                    for (int s = 0; s < CaliValue.Count; s++)
                    {
                        ...
                        if (!com.MixReadStCurValue(0x03, CurRange, out CellCurCaliTesterValue1))
                            return false;
                        ...
                        UpdateUidelegate(DeviceType, Box, "STC_2000nA点检", CaliValue[s].ToString(), "", CellCurCaliTesterValue1.ToString(), Err1.ToString("f2"), "", CurCheckAcc.ToString(), Result);
                        if (s == CaliValue.Count - 1)
                        {
