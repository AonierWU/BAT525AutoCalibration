...
namespace TestSystem_Pack
{
    ...
    public partial class frmVerifyDevice : Form
    {
        ...
        private static string CheckReportPath = "";
        private static XSSFWorkbook xssfWorkbook;
        private static XSSFSheet xssfSheet;
        private static XSSFRow xssfRow;
        private static XSSFCell xssfCell;
        private static XSSFRow xssfRow2;
        private static XSSFCell xssfCell2;
        private static XSSFRow xssfRow3;
        private static XSSFCell xssfCell3;
        ...
        */
        ...
        private void SaveBarCode(string BarCode)
        {
            try
            {
                if (this.InvokeRequired)
                {
                    this.Invoke(new SaveBarCodeDelegate(SaveBarCode), BarCode);
                }
                else
                {
                    CheckReportPath = Application.StartupPath + "\\BAT525H点检报告.xlsx";
                    ...
                    xssfCell.SetCellValue(BarCode);
                    string NewFileName = BarCode.Substring(15, 4) + "_" + BarCode.Substring(0, 4) + "_测试设备出货检测表单.xlsx";
                    CheckReportPath = Application.StartupPath + @"\TestData" + @"\" + NewFileName;
        ...
        int visitCount = 0;
        private void WriteDataToReport(int[,] Cell, string DMMValue, string EQMValue, string ChkAcc)//增加委托;1.int[,] 0位是Row,1位是Cell
        {
            try
            {
                if (this.InvokeRequired)
                {
                    this.Invoke(new Action(() =>
                    {
                        ...
                        xssfCell.SetCellValue(DMMValue);
                        xssfRow2 = (XSSFRow)xssfSheet.GetRow(Cell[1, 0]);
                        xssfCell2 = (XSSFCell)xssfRow2.GetCell(Cell[1, 1]);
                        xssfCell2.SetCellValue(EQMValue);
                        xssfRow3 = (XSSFRow)xssfSheet.GetRow(Cell[2, 0]);
                        xssfCell3 = (XSSFCell)xssfRow3.GetCell(Cell[2, 1]);
                        xssfCell3.SetCellValue(ChkAcc);
